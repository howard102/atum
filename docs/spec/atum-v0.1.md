...
**十六條原則的權威在 [`docs/constitution.md`](../constitution.md)，本節刻意不複製。**

理由：條文若同時存在 spec 與 `atum.yaml` 兩處，必然開始漂移——這正是
`docs/tech/kratos-reference.md` §3 第 5 項記錄的 Kratos 失敗模式。
`atum.yaml` 只記 `constitution.count`，由 `scripts/check-consistency.py` 驗證兩處一致。
